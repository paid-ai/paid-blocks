...
export const PaidInvoiceTable: React.FC<PaidInvoiceTableProps> = ({
  ...
  const [PaymentModal, setPaymentModal] = useState<React.ComponentType<any> | null>(null);
  ...
  // Dynamically load payment modal when payments are enabled
  useEffect(() => {
    if (enablePayments && stripePublishableKey && !PaymentModal) {
      import('../payments/index').then((module) => {
        setPaymentModal(() => module.InvoicePaymentModal);
      }).catch((err) => {
        console.error('Failed to load payment module. Make sure @stripe packages are installed:', err);
      });
    }
  }, [enablePayments, stripePublishableKey, PaymentModal]);
  ...
  return (
    <>
                    ...
                    <tr key={invoice.id}>
                      ...
                      <td style={{ textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                          ...
                          {invoice.paymentStatus === 'pending' && enablePayments && stripePublishableKey && (
                            <button
                              className="paid-invoice-action-btn paid-invoice-pay-btn"
                            ...
                            >
                              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                                ...
                                <path d="M3 9h3" stroke="currentColor" strokeWidth="1.5" />
                              </svg>
                            </button>
                          )}
                        </div>
      ...
      {/* Payment Modal */}
      {paymentModalInvoice && PaymentModal && stripePublishableKey && (
        <PaymentModal
          ...
          onSuccess={() => {
            ...
            window.dispatchEvent(new CustomEvent('cache-refresh', { detail: { customerId: customerExternalId } }));
